...
let bulkSocket;
let selectedEmailColumns = new Set();
const RADIUS = 90;
let deleteUUID = null;
...
$(document).ready(function () {
  ...
  $('input[name="email_first_row_label"]').on('change', function () {
  ...
  $(document).on('click', '#submitUpload' ,function () {
    ...
    reader.onload = function (evt) {
        ...
        const emailColumns = detectEmailColumns(headers, rowValues);
    
        $('#emailColumns').text(
            emailColumns.length ? emailColumns.join(', ') : 'None'
        );

        if (!emailColumns.length) {
            showError('File doesnot contains any emails.');
...
//     if (selectedEmailColumns.has(index)) {
//         selectedEmailColumns.delete(index);
//     } else {
//         selectedEmailColumns.add(index);
//     }

//     updateHeaderUI();
//     updateHiddenInput();
//     // $('#selected_email_column').val([...selectedEmailColumns].join(','));
// });
...
$(document).on("click", ".status-tab[data-status]", function () {
  ...
  $('#confirmDeleteYes').on('click', function () {
    ...
    $.ajax({
        url: `/api/bulk-validation/${deleteUUID}/delete/`,
        type: 'DELETE',
        success: function () {
            location.reload();
        },
...
$(document).on('click', '.delete-close-modal', function () {
  ...
  $(document).on('click', '.initiate-verification',function () {
...
$.ajax({
  ...
  // Handle upload submit
  $('.validate-emails').on('click', function () {
  ...
  $('.custom-scrollbar').on('scroll', function () {
   ...
   $.ajax({
    ...
    $(document).on('click', '.add-multiple-emails', function () {
      $('#addEmailsModal').removeClass('hidden');
...
// startBulkStatusPolling();
  ...
  class ChunkUploader {
    ...
    formData.append('first_row_label', $('input[name="email_first_row_label"]:checked').val());
    formData.append('duplicate_confirm', $('input[name="email_duplicate_confirm"]:checked').val());
    formData.append('selected_file_columns', $('#selected_email_column').val());
...
function fetchData() {
    $.ajax({
        url: `/api/bulk-validation-list/?page=${currentPage}&status=${currentStatus}`,
        type: "GET",
        success: function (html) {
            $("#bulkListDiv").html(html);
        },
        error: function (xhr) {
...
function closeModal() {
    document.getElementById('addEmailsModal').classList.add('hidden');
    ...
    function handleContinue() {
      ...
      // Step 2: invalid emails found — show errors and STOP
      if (invalid.length > 0) {
    ...
    $('#previewState').addClass('hidden');
        ...
        $.ajax({
            url: '/ajax/process-emails/',
            type: 'POST',
            data: {
                emails: uniqueValid.join(','),
            },
            success: function (res) {
                $('.progress-bar').css('width', '100%');
                $('.progress-info').text('100%');
...
document.getElementById('addEmailsModal').addEventListener('click', function(e) {
...
function handleExportOptions(value) {
        const $emailStatus = $('.export-option');
        const $emailCheckbox = $emailStatus.find('.export-checkbox');
        const $emailCheckboxSVG = $emailStatus.find('svg');
        const $emailCheckboxText = $emailStatus.find('.export-checkbox-text');

        if (value === 'no') {
            // Force Email Status checked & disabled
            $emailCheckbox.prop('disabled', false);
            $emailCheckboxSVG.attr('stroke','currentColor')

            // $emailStatus
            //     .addClass('opacity-40 cursor-not-allowed')
            //     .find('.check-icon').removeClass('hidden');
            $emailCheckboxText.removeClass('opacity-40 cursor-not-allowed')

        } else {
            // Enable everything back
            $emailCheckbox.prop('checked', true).prop('disabled', true);
            $emailCheckboxSVG.attr('stroke','#b5b5b8')
            // $emailStatus.removeClass('opacity-40 cursor-not-allowed');
            $emailCheckboxText.addClass('opacity-40 cursor-not-allowed')

        $emailCheckboxSVG.removeClass('hidden');
        }
    }

function initBulkSocket() {
    if (bulkSocket) return;

    const protocol = location.protocol === 'https:' ? 'wss://' : 'ws://';
    bulkSocket = new WebSocket(`${protocol}${location.host}/ws/bulk-dashboard/`);

    bulkSocket.onmessage = (e) => {
...
function detectEmailColumns(headers, rows) {
    const columnStats = {};
    const sampleSize = Math.min(rows.length, 20);

    // Initialize stats
    headers.forEach((_, colIndex) => {
        columnStats[colIndex] = { total: 0, matches: 0 };
    });
    ...
    // Return column indexes + names where ≥50% are emails
    return headers
        .map((header, index) => {
            const stat = columnStats[index];
            return stat.matches > 0 && stat.matches / stat.total >= 0.5
                ? { index, header }
                : null;
        })
        .filter(Boolean);
...
function renderPreview(headers, rows, previewLimit = 5) {
    ...
    let detectedEmailIndex = -1;

    // Auto-detect email column (scan first N rows)
    headers.forEach((_, colIndex) => {
        let count = 0;
        rows.slice(0, previewLimit).forEach(row => {
            if (emailRegex.test((row[colIndex] || '').trim())) {
                count++;
            }
        });
    
        if (count >= 2) {
            selectedEmailColumns.add(colIndex);
        }
    });

    let theadHtml = '<tr class="bg-gray-50/50">';
    headers.forEach((header, index) => {
        const isEmail = index === detectedEmailIndex;
        ...
        <th class="p-4 border-b border-gray-100 text-left">
            <div class="flex flex-col gap-3">
                <div data-index="${index}" class="flex items-center justify-center px-3 py-2 rounded-lg border transition-all column-selector
                    ...
                    <span class="text-base font-bold uppercase tracking-wider truncate pr-2">
                        ${isEmail ? 'Email Address' : 'Ignore Column'}
                    </span>
    ...
    // Header-name row (strikethrough)
    tbodyHtml += '<tr class="border-b border-gray-50/50">';
    headers.forEach(h => {
        tbodyHtml += `
        <td class="p-4">
            <div class="text-base truncate max-w-[200px] text-gray-300 italic line-through header-row-strike decoration-gray-200">
                ${h}
            </div>
        </td>`;
    });
    ...
    rows.slice(0, previewLimit).forEach(row => {
        ...
        row.forEach((cell, index) => {
            tbodyHtml += `
            <td class="p-4">
                <div class="text-base truncate max-w-[200px]
                    ${index === detectedEmailIndex
    ...
    // Persist detected email column
    if (detectedEmailIndex !== -1) {
        $('#selected_email_column').val(detectedEmailIndex);
        $('.validate-emails').removeAttr('disabled')
    }
    else{
        $('.validate-emails').attr('disabled',true)
    }
}
...
function updateHiddenInput() {
    $('#selected_email_columns').val([...selectedEmailColumns].join(','));
    
    if ([...selectedEmailColumns].length !== 0) {
        $('.validate-emails').removeAttr('disabled')
    }
    else{
        $('.validate-emails').attr('disabled',true)
...
function updateHeaderUI() {
    $('.column-selector').each(function () {
        ...
        const isEmail = selectedEmailColumns.has(index);
        ...
        $icon
            .toggleClass('bg-white border-white', isEmail)
            .toggleClass('bg-transparent border-gray-300', !isEmail)
            .html(isEmail ? CHECK_ICON_SVG : '');
    });
}
...
function startBulkStatusPolling() {
    if (bulkPollingInterval) return;

    bulkPollingInterval = setInterval(function () {
...
function stopBulkStatusPolling() {
    ...
    console.log('Bulk polling stopped');
}
...
function handleBulkStatsResponse(apiResponse) {
...
function fetchBulkJobStatuses(jobIds) {
    $.ajax({
        url: '/api/bulk-validation/',
        type: 'POST',
        contentType: 'application/json',
        data: JSON.stringify({ job_ids: jobIds }),
        success: function (response) {
            handleBulkStatsResponse(response);
        stopIfAllCompleted();
        },
        error: function () {
            console.error('Bulk status fetch failed');
        }
    });
}
...
function setSegment(job_id, id, length, offset) {
  ...
  function updateDonutChart(job_id, stats) {
    ...
    const segments = [
      ...
      { id: "seg-risky", value: stats.risky },
