...
class UserToken(models.Model):
    ...
    TOKEN_TYPE_CHOICE = (
        (FORGOT_OTP_WEB, 'Forgot password OTP'),
        (FORGOT_LINK_WEB, 'Forgot password Link')
    )
    ...
    def create_token(token_type, user=None, extras=None):
        '''
            Create Token
        '''
        if token_type == UserToken.FORGOT_OTP_WEB:
    ...
    def send_forgot_password_email(self, request):
        ...
        domain = request.get_host()
        protocol = request.scheme
        if self.token_type == UserToken.FORGOT_OTP_WEB:
