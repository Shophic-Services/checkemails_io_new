(function( $ ){
	...
	$.fn.multiple_emails = function(options) {
		...
		{
			deleteIconHTML = '<a href="#" class="multiple_emails-close" title="Remove"><span class="email_address"></span></a>';
		}
      ...
      <div class="card-body accordion-body">
			...
			if ($(this).val() != '' && IsJsonString($(this).val())) {
				$.each(jQuery.parseJSON($(this).val()), function( index, val ) {
					$list.find('ul').append($('<li class="multiple_emails-email"><span class="email_name" data-email="' + val.toLowerCase() + '">' + val + '</span></li>')
			...
			var $input = $('<input type="text" class="multiple_emails-input text-left form-control email-search__input" placeholder="' + settings.placeholder +'"/>').on('keyup', function(e) { // input
				...
				if ($(this).val() != '') { display_email($(this), settings.checkDupEmail); 
					if($(event.relatedTarget).hasClass('email-search__submit')){

						$('.email-search__submit').trigger('click')
					}
			...
			*/
			...
			function display_email(t, dupEmailCheck) {
				...
				for	(var i = 0; i < arr.length; i++) {
					...
					if ( dupEmailCheck === true && $orig.val().indexOf(arr[i]) != -1 ) {
						...
						$list.find('ul').append($('<li class="multiple_emails-email"><span class="email_name" data-email="' + arr[i].toLowerCase() + '">' + arr[i] + '</span></li>')
							  .prepend($(deleteIconHTML)
								   .click(function(e) { $(this).parent().remove(); refresh_emails(); e.preventDefault(); })
							  )
						);
			...
			function refresh_emails () {
				...
				if($list.find('ul li').length > 0){
					placeholder_value = ($list.find('ul li').length).toString() + ' Emails Entered'
					$('.accordion').removeClass('d-none')
				}
				else{
					placeholder_value = settings.placeholder
					$('.accordion').addClass('d-none')
				}
				container.find('.multiple_emails-input.email-search__input').attr('placeholder',placeholder_value)
			}
